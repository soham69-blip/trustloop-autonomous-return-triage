import pandas as pd
import numpy as np
import pickle

from sklearn.metrics import (
    accuracy_score,
    precision_score,
    recall_score,
    f1_score
)

DATA_PATH = "data/processed/trustloop/model_ready.csv"
MODEL_PATH = "models/lightgbm_model.pkl"

print("=" * 70)
print("TRUSTLOOP - POLICY ABUSER THRESHOLD EXPERIMENT")
print("=" * 70)

df = pd.read_csv(DATA_PATH)

TARGET = "abuse_label"

categorical = [
    "country",
    "customer_segment",
    "device_type",
    "payment_method",
    "platform",
    "product_category",
    "return_reason",
    "shipping_carrier"
]

for col in ["order_date", "return_date"]:
    df[col] = pd.to_datetime(df[col], errors="coerce")

def add_temporal_features(data, col):
    data[col + "_year"] = data[col].dt.year
    data[col + "_month"] = data[col].dt.month
    data[col + "_day"] = data[col].dt.day
    data[col + "_dayofweek"] = data[col].dt.dayofweek
    data[col + "_dayofyear"] = data[col].dt.dayofyear
    data[col + "_is_weekend"] = (
        data[col].dt.dayofweek.isin([5, 6])
    ).astype(int)

add_temporal_features(df, "order_date")
add_temporal_features(df, "return_date")

df["calculated_days_to_return"] = (
    df["return_date"] - df["order_date"]
).dt.total_seconds() / 86400

feature_cols = [
    c for c in df.columns
    if c not in [TARGET, "order_date", "return_date"]
]

for col in categorical:
    if col in df.columns:
        df[col] = df[col].astype("category")

X = df[feature_cols]
y = df[TARGET].astype(int)

n = len(df)

train_end = int(n * 0.70)
val_end = int(n * 0.85)

X_test = X.iloc[val_end:].copy()
y_test = y.iloc[val_end:].copy()

with open(MODEL_PATH, "rb") as f:
    model = pickle.load(f)

proba = model.predict_proba(X_test)

normal_prediction = np.argmax(proba, axis=1)

thresholds = [
    0.30,
    0.35,
    0.40,
    0.45,
    0.50,
    0.55,
    0.60,
    0.65
]

results = []

print()
print("BASELINE")
print("-" * 70)

baseline_accuracy = accuracy_score(
    y_test,
    normal_prediction
)

baseline_macro_f1 = f1_score(
    y_test,
    normal_prediction,
    average="macro"
)

baseline_policy_precision = precision_score(
    y_test,
    normal_prediction,
    labels=[1],
    average="macro",
    zero_division=0
)

baseline_policy_recall = recall_score(
    y_test,
    normal_prediction,
    labels=[1],
    average="macro",
    zero_division=0
)

baseline_policy_f1 = f1_score(
    y_test,
    normal_prediction,
    labels=[1],
    average="macro",
    zero_division=0
)

print("Accuracy:", round(baseline_accuracy, 4))
print("Macro F1:", round(baseline_macro_f1, 4))
print("Policy Precision:", round(baseline_policy_precision, 4))
print("Policy Recall:", round(baseline_policy_recall, 4))
print("Policy F1:", round(baseline_policy_f1, 4))

for threshold in thresholds:

    pred = normal_prediction.copy()

    policy_probability = proba[:, 1]

    policy_candidates = policy_probability >= threshold

    pred[policy_candidates] = 1

    accuracy = accuracy_score(
        y_test,
        pred
    )

    macro_f1 = f1_score(
        y_test,
        pred,
        average="macro"
    )

    policy_precision = precision_score(
        y_test,
        pred,
        labels=[1],
        average="macro",
        zero_division=0
    )

    policy_recall = recall_score(
        y_test,
        pred,
        labels=[1],
        average="macro",
        zero_division=0
    )

    policy_f1 = f1_score(
        y_test,
        pred,
        labels=[1],
        average="macro",
        zero_division=0
    )

    results.append({
        "threshold": threshold,
        "accuracy": accuracy,
        "macro_f1": macro_f1,
        "policy_precision": policy_precision,
        "policy_recall": policy_recall,
        "policy_f1": policy_f1
    })

results_df = pd.DataFrame(results)

results_df.to_csv(
    "reports/policy_threshold_experiment.csv",
    index=False
)

print()
print("=" * 70)
print("THRESHOLD RESULTS")
print("=" * 70)

print(
    results_df.round(4).to_string(index=False)
)

print()
print("Saved:")
print("reports/policy_threshold_experiment.csv")

print()
print("=" * 70)
print("THRESHOLD EXPERIMENT COMPLETE")
print("=" * 70)
